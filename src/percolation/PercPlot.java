...
public class PercPlot {
    ...
    public static void curve(int Number, double x0, double y0, double x1, double y1) {
        double gap = .01;
        double err = .0025;
        int M = 10000;
        double xm = (x0 + x1) / 2;
        double ym = (y0 + y1) / 2;
        double fxm = Estimate.eval(Number, xm, M);
        if (x1 - x0 < gap || Math.abs(ym - fxm) < err) {
            StdDraw.line(x0, y0, x1, y1);
            return;
        }
        curve(Number, x0, y0, xm, fxm);
        StdDraw.filledCircle(xm, fxm, .005);
        curve(Number, xm, fxm, x1, y1);
    }
